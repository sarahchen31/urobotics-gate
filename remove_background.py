...
def eqHisthsv(image_in, saturation=False):
    image_bgr = image_in.copy()
    img_hsv = cv.cvtColor(image_bgr, cv.COLOR_BGR2HSV)
...
def try_1(img):
    ...
    for i in range(rows):
        for j in range(cols):
            if img.item(i, j, 2) < img.item(i, j, 0) and img.item(i, j, 2) < img.item(i, j, 1):
                img.itemset((i, j, 0), 0)
                img.itemset((i, j, 1), 0)
                img.itemset((i, j, 2), 0)
...
def filter_local_maxima(frame):
    ret = np.zeros((frame.shape[0], frame.shape[1], 3))
    f = frame[:, :, 2].flatten()
    maxIdx = argrelextrema(f, np.greater)
    for i in maxIdx[0]:
        row, col = i // frame.shape[1], i % frame.shape[1]
        ret.itemset((row, col, 0), 255)
        ret.itemset((row, col, 1), 255)
        ret.itemset((row, col, 2), 255)
    return ret
...
while(cap.isOpened()):
    ...
    if ret:
        ...
        #fgMask = backSub.apply(masked_filter)

        masked_filter = analyse(multi_filter2)
        
        #local_maxima = filter_local_maxima(frame)
        #denoised_local_maxima = denoised_filter_local_maxima(frame)

        #try_1(frame)

        #image = frame.copy()
        #get_single_channel_frame(image, 'r')
